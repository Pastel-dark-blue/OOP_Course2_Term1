...
namespace Laba2
{
    class Program2
    {
        ...
        {
            //*********************************РАССКОМЕНТИРОВАТЬ ПО БЛОКАМ ЗАДАНИЙ*********************************
            ...
            //var a = 5;
            //var b = "I was born on the wrong side of the train tracks";
            //var c = 9.87f;
            ...
            //``````````````````` ЗАДАНИЕ 2 (d) ```````````````````
            //StringBuilder str = new StringBuilder("Can you please sit the fuck down"); //выделяет больше памяти, чем нужно в данный момент, для возможности менять строку

            //Console.WriteLine($"Remove : {str.Remove(0, 23)}");

            //str.Insert(0, "->");
            //str.Append("<-");
            //Console.WriteLine($"Insert + Append: {str}");
            ...
            //``````````````````` ЗАДАНИЕ 3 (d) ```````````````````
            //var str = "From Today on, It's My Turn!!";
            //var arr = new[] { 1.2, 7.09, 8.3 };
